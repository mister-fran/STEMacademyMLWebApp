...
def main():
    ...
    dataset = st.sidebar.radio("Vælg et datasæt:", ["Gletsjer", "Partikel"])
    ...
    if path.exists(DATA_PATHS['VejledningHUSPRISER']):
        try:
    ...
    if path.exists(DATA_PATHS['VejledningDIABETES']):
        try:
    ...
    if path.exists(DATA_PATHS['VejledningGLETSJER']):
        try:
    ...
    if path.exists(DATA_PATHS['VejledningPARTIKEL']):
        try:
